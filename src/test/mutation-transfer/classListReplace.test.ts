...
test.serial('Element.classList.replace transfer single pre-existing value', async (t) => {
...
test.serial('Element.classList.replace transfer multiple pre-existing values', async (t) => {
  const { document, emitter } = t.context;
  const div = document.createElement('div');
