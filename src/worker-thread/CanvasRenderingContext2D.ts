...
function getGoodOffscreenCanvas(): Promise<{getContext(c: '2d'): CanvasRenderingContext2DImplementation}> {
    return Promise.resolve(new OffscreenCanvas(10, 10) as {getContext(c: '2d'): CanvasRenderingContext2DImplementation});
}

export class CanvasRenderingContext2D implements CanvasRenderingContext2DImplementation {
    // store all method calls (getter/setter calls) in a temporary buffer
    ...
    constructor() {
        // attempt to create an auto-synchronizing OffscreenCanvas
        // replace with polyfill if necessary
        getGoodOffscreenCanvas().then((instance) => {
    ...
    set font(value: string /* THIS IS SUPPOSED TO BE A DOMString! */) {
    ...
    // Either "start" (default), "end", "left", "right", or "center"
    ...
    // Either "top", "hanging", "middle", "alphabetic" (default), "ideographic", or "bottom"
    ...
    */
    ...
    /* DRAWING IMAGES */
    // method overloading: a bunch of optional arguments in some method signatures
    ...
    /* PIXEL MANIPULATION */
    // more method overloading (createImageData(ImageData))
    ...
    //    return this.implementation.canvas;
    //}
...
// All method calls will be delegated to an 'implementation' of
// OffscreenCanvas that does the actual rendering
