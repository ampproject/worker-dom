...
/**
 * RollupPlugin that removes the null checks for Window in Main Thread Code (where a Window always exists).
 */
export function removeWindowNullChecks() {
  let context;

  return {
    name: 'remove-window-null-checks',
    buildStart() {
      context = this;
    },
    async renderChunk(code) {
      ...
      const program = context.parse(code, { ranges: true });
      const toOverwrite = [];
      let foundAndRemoved = 0;

      walk.simple(program, {
        ClassDeclaration(node) {
          if (node.id.name === 'WorkerContext') {
            foundAndRemoved = foundAndRemoved + 1;

            walk.simple(node.body, {
              LogicalExpression(node) {
                if (node.operator === '||' && node.left.type === 'LogicalExpression' && node.right.type === 'Literal') {
                  toOverwrite.push([node.range[0], node.range[1], code.substring(node.left.right.range[0], node.left.right.range[1])]);
                  foundAndRemoved = foundAndRemoved - 1;
                }
              },
            });
      ...
      if (foundAndRemoved > 0) {
        context.warn(`${foundAndRemoved} Window Null Checks were found but not removed during compilation.`);
      }

      if (toOverwrite.length > 0) {
        toOverwrite.forEach(tuple => {
          source.overwrite(tuple[0], tuple[1], tuple[2]);
        });
      }
